...
bot.on("callback_query:data", async (ctx) => {
  ...
  try {
    ...
    // Проверка админа ТОЛЬКО для административных кнопок
    if (chatId !== ADMIN_ID) {
...
bot.on("message:text", async (ctx) => {
  ...
  try {
    ...
    if (existingUser) {
      ...
      // Email не найден - предлагаем зарегистрироваться
      await ctx.reply(
        `❌ Користувача з email ${emailNormalized} не знайдено.\n\n` +
          `💡 <b>Якщо ви вже створили бронювання:</b>\n` +
          `1. Зареєструйтеся на сайті, вказавши ті самі дані (email/телефон), що й при бронюванні\n` +
          `2. Поверніться сюди та спробуйте авторизуватися ще раз\n\n` +
          `Або натисніть кнопку '📱 Поділитися номером' для автоматичної реєстрації.`,
        { parse_mode: "HTML" }
      );
    }
  } catch (error) {
